...
Active support for PHP 8.3 ended December 31st, 2025 and PHP 8.4 will have active support until December 31st, 2026. I have decided to go ahead and
push toward PHP 8.4.

### Changed

* Bumped minimum PHP version to PHP 8.4.
* Updated `SECURITY.md`.
* [BC-BREAK] The `getters` and `setters` in `Esi\CoverageCheck\CoverageCheck` no longer exist. Access the properties directly instead. See removed below.
* [BC-BREAK] `protected` constants in `Esi\CoverageCheck\CoverageCheck` changed from `protected` to `private`.

### Removed

* [BC-BREAK] Removed `getCloverFile()`, `setCloverFile()`, `getOnlyPercentage()`, `setOnlyPercentage()`, `getThreshold()`, and
  `setThreshold()` in `Esi\CoverageCheck\CoverageCheck`. Access the properties directly instead.
...
[3.1.0]: https://github.com/ericsizemore/phpunit-coverage-check/releases/tag/3.1.0
